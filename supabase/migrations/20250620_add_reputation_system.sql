...
-- This migration safely adds reputation tables without affecting existing user data
...
CREATE TABLE IF NOT EXISTS reputation_events (
    ...
    gig_id UUID, -- References gigs(id) but no foreign key to avoid migration issues
...
-- Policies for authenticated users (only create if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'legal_credentials' AND policyname = 'Users can view their own credentials') THEN
        CREATE POLICY "Users can view their own credentials" ON legal_credentials FOR SELECT USING (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'legal_credentials' AND policyname = 'Users can insert their own credentials') THEN
        CREATE POLICY "Users can insert their own credentials" ON legal_credentials FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'legal_credentials' AND policyname = 'Users can view public credentials') THEN
        CREATE POLICY "Users can view public credentials" ON legal_credentials FOR SELECT USING (verification_status = 'verified');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'reputation_scores' AND policyname = 'Users can view their own reputation scores') THEN
        CREATE POLICY "Users can view their own reputation scores" ON reputation_scores FOR SELECT USING (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'reputation_scores' AND policyname = 'Anyone can view reputation scores') THEN
        CREATE POLICY "Anyone can view reputation scores" ON reputation_scores FOR SELECT USING (true);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'reputation_events' AND policyname = 'Users can view reputation events') THEN
        CREATE POLICY "Users can view reputation events" ON reputation_events FOR SELECT USING (
            auth.uid() = user_id OR auth.uid() = reviewer_id
        );
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'reputation_events' AND policyname = 'Users can insert reputation events') THEN
        CREATE POLICY "Users can insert reputation events" ON reputation_events FOR INSERT WITH CHECK (
            auth.uid() = user_id OR auth.uid() = reviewer_id
        );
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'legal_case_completions' AND policyname = 'Users can view relevant case completions') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'reputation_attestations' AND policyname = 'Users can view attestations about themselves') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'reputation_attestations' AND policyname = 'Users can create attestations') THEN
...
-- Trigger to update reputation scores (only create if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_reputation_scores_trigger') THEN
...
-- Insert default reputation configuration (only if it doesn't exist)
INSERT INTO reputation_config (config_type, config_data, description) 
SELECT * FROM (VALUES 
    ('scoring_weights', '{"gig_completion": 5.0, "positive_review": 3.0, "property_approval_accuracy": 4.0, "dispute_resolution": 6.0, "peer_endorsement": 2.0, "credential_verification": 1.5}'::jsonb, 'Default scoring weights for reputation events'),
    ('rating_multipliers', '{"1_star": 0.2, "2_star": 0.4, "3_star": 0.6, "4_star": 0.8, "5_star": 1.0}'::jsonb, 'Rating multipliers for score calculation'),
    ('reputation_thresholds', '{"novice": 0, "competent": 25, "proficient": 50, "expert": 75, "master": 90}'::jsonb, 'Reputation level thresholds')
) AS t(config_type, config_data, description)
WHERE NOT EXISTS (
    SELECT 1 FROM reputation_config WHERE config_type = t.config_type
);

-- Add comment to track migration
COMMENT ON TABLE legal_credentials IS 'Reputation system migration applied 2025-06-20 - preserves existing user data';